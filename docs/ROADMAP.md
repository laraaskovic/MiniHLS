...
| **S3** | **Fixed-width integer support.** Wrap, truncate, sign- and zero-extend at arbitrary widths. Used by the interpreter and every testbench. Note the consequence of [LANGUAGE.md](../LANGUAGE.md)'s two width limits: written types stop at 64 bits but expression results reach 128, so runtime values need a 128-bit representation — wider than anything a program can declare — and constant folding needs arbitrary precision on top of that, since untyped constants are exact before they acquire a type. *Done when:* unit tests cover the edge widths (1, 63, 64, 65, 127, 128 bits). | S |
